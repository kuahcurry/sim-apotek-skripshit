...
export default function PemusnahanEdit({ batches, users, pemusnahan }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        tanggal_pemusnahan: pemusnahan.tanggal_pemusnahan,
        penanggung_jawab: pemusnahan.penanggung_jawab.toString(),
        lokasi_pemusnahan: pemusnahan.lokasi_pemusnahan || '',
        metode_pemusnahan: pemusnahan.metode_pemusnahan || '',
        saksi: (pemusnahan.saksi && pemusnahan.saksi.length > 0) 
            ? pemusnahan.saksi 
            : [{ nama: '', jabatan: '' }] as SaksiItem[],
        alasan: pemusnahan.alasan,
        keterangan: pemusnahan.keterangan || '',
        status: pemusnahan.status,
        details: pemusnahan.details.map(detail => ({
            batch_id: detail.batch_id.toString(),
            jumlah: detail.jumlah.toString(),
            nilai_perolehan: detail.nilai_perolehan.toString(),
    ...
    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        put(`/pemusnahan/${pemusnahan.id}`, {
            onSuccess: () => {
                console.log('Pemusnahan updated successfully');
            },
            onError: (errors) => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Pemusnahan - ${pemusnahan.nomor_berita_acara}`} />
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Edit Pemusnahan Obat</h1>
                        <p className="text-sm text-muted-foreground">
                            Edit berita acara pemusnahan obat: {pemusnahan.nomor_berita_acara}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="max-w-5xl space-y-6">
                    ...
                    <div className="flex gap-2">
                        <Button type="submit" disabled={processing}>
                            ...
                            {processing ? 'Mengupdate...' : 'Update Pemusnahan'}
                        </Button>
